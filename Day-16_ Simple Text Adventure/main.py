...
def downstairs(has_key):
    ...
    action = get_valid_input(
        "What do you do? (fight / run / use key): ", ["fight", "run", "use key"]
    )
    if action == "fight":
        ...
        if has_key:
            print(
                "You hold up the shiny key. The dragon steps back and reveals a hidden exit.\nYou escape and win!"
            )
        else:
...
def open_another_door(has_key):
    ...
    action = get_valid_input(
        "Do you want to go down the stairs or return to the hallway? (downstairs / return): ",
        ["downstairs", "return"],
    )
...
def left_door():
    ...
    action = get_valid_input(
        "What do you want to do? (pick up key / leave): ", ["pick up key", "leave"]
    )
    has_key = False
    if action == "pick up key":
        has_key = pickup_key()
        another_door = get_valid_input(
            "There is another door in the room. Do you want to open it? (yes / no): ",
            ["yes", "no"],
        )
        if another_door == "yes":
        ...
        print(
            "You leave the room without taking the key, possibly making it harder to progress."
        )
    return has_key
...
def enter_the_room():
    ...
    direction = get_valid_input(
        "Do you want to go left or right? (left / right): ", ["left", "right"]
    )
    if direction == "left":
        has_key = left_door()
...
def dungeon():
    print(
        "Welcome, adventurer! You find yourself at the entrance of a dark and mysterious dungeon."
    )
